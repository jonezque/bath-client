...
export class SittingPlacesWomenComponent implements OnInit {
  ...
  createOrder() {
    ...
    dialogRef.beforeClose().subscribe(res => {
      if (res) {
        this.db.createOrder({ ...res, room: 'women' }).subscribe();
  ...
  changePlaces() {
    ...
    if (selected.length) {
      ...
      dialogRef.beforeClose().subscribe(res => {
        if (res) {
          this.db.exchangePlaces(p1.name, p2.name, 'women').subscribe();
          this.removeSelection();
        }
      });
    }
  ...
  addTime() {
    ...
    this.db.addTime(ids, 'women').subscribe();
    this.removeSelection();
  }
      ...
      dialogRef.beforeClose().subscribe(res => {
        if (res) {
          if (!res.error) {
            ...
            this.db.cancelOrders({ ...res, room: 'women' }).subscribe();
            this.removeSelection();
          }
        }
      });
  ...
  private initPositions() {
    this.db.getBusyPlaces('women').subscribe(res =>
  ...
  private updatePrice() {
    ...
    .pipe(
      flatMap(x => x),
      filter(x => x.room === RoomType.women && x.type === PlaceType.cab))
  ...
  private setFreePlaces(ids: any) {
    this.db.freePlaces(ids, 'women').subscribe();
    this.removeSelection();
  }
}
